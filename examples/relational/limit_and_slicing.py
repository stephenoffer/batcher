...
def main() -> None:
    ...
    assert orders.limit(5).count() == 5
    assert orders.limit(5).to_pydict() == orders.limit(5).to_pydict()
    ...
    page_two = orders.limit(5, offset=5).to_pydict()
    first_ten = orders.limit(10).to_pydict()
    assert page_two["o_orderkey"] == first_ten["o_orderkey"][5:]

    # Every nth row, for a cheap deterministic thinning of a large scan.
    thinned = orders.limit(20).gather_every(4).to_pydict()
    print("thinned:", thinned["o_orderkey"])
    assert (
        thinned["o_orderkey"]
        == first_ten["o_orderkey"][:1] + orders.limit(20).to_pydict()["o_orderkey"][4::4]
    )
    ...
    assert orders.first(named=True)["o_orderkey"] == orders.limit(1).to_pydict()["o_orderkey"][0]

    # `item` is the scalar form, and only valid on a single cell.
    biggest = orders.sort("o_totalprice", descending=True).select("o_totalprice").limit(1)
