...
The random ones take a seed, because an unseeded split is a split you cannot reproduce
when the result looks wrong. ``sample_per_group`` is the exception and needs none: it caps
each group by an ordering rather than by chance, so it is already deterministic.
``stratified_split`` preserves class balance; a plain random split does not, and on an
imbalanced problem that matters.
