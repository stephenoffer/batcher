...
{doc}`Sort internals </deep-dives/operators/sort-internals>`.
...
- {doc}`Performance </user-guide/operate/performance>`: the `morsel_rows` and `parallelism` knobs, applied.
...
- {doc}`Mergeable algebra </deep-dives/operators/mergeable-algebra>`: why the parallel schedule computes the same answer.
- {doc}`Join algorithms </deep-dives/operators/join-algorithms>`: the shuffle-and-bucket join in detail.
- {doc}`Arrow and memory </deep-dives/memory/arrow-memory>`: the byte budget a morsel is bounded by.
