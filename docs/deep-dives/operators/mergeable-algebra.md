...
- {doc}`Aggregation internals </deep-dives/operators/aggregation-internals>`: how `partial` and `combine` actually run.
- {doc}`Morsel parallelism </deep-dives/operators/morsel-parallelism>`: the scheduling this algebra makes safe.
- {doc}`Distributed scheduling </deep-dives/distribution/distributed-scheduling>`: the same three functions, mapped over Ray.
