...
commutative) is covered in {doc}`Mergeable algebra </deep-dives/operators/mergeable-algebra>`. This page covers what
...
- {doc}`Distinct and dedup </user-guide/transform/distinct-and-dedup>`: the `DISTINCT` surface.
- {doc}`Analytics benchmarks </benchmarks/analytics>`: the group-by numbers quoted above.
- {doc}`Mergeable algebra </deep-dives/operators/mergeable-algebra>`: why any of this is allowed to run in parallel.
- {doc}`Morsel parallelism </deep-dives/operators/morsel-parallelism>`: where the morsels come from.
- {doc}`Spilling </deep-dives/memory/spilling>`: what happens when the state does not fit.
