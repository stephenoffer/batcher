...
- {doc}`The shuffle over Arrow Flight </deep-dives/distribution/shuffle-flight>`: what the credits are gating.
- {doc}`The buffer pool </deep-dives/memory/buffer-pool>`: where `PressureLevel` comes from.
- {doc}`Distributed scheduling </deep-dives/distribution/distributed-scheduling>`: how many channels there are.
