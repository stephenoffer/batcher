...
- {doc}`Mergeable algebra </deep-dives/operators/mergeable-algebra>`: why a partitioned result equals a single-node one.
- {doc}`Shuffle over Arrow Flight </deep-dives/distribution/shuffle-flight>`: how the bytes actually move.
- {doc}`Credit-based flow control </deep-dives/distribution/credit-flow-control>`: what keeps a reducer from drowning.
- {doc}`Learned metadata </deep-dives/adaptive/learned-metadata>`: where the learned partition counts live.
