...
- {doc}`GPU execution </deep-dives/distribution/gpu-execution>`: the two paths that run work on a device.
- {doc}`Shuffle over Arrow Flight </deep-dives/distribution/shuffle-flight>`: why bulk data bypasses the Ray object store.
- {doc}`Credit-based flow control </deep-dives/distribution/credit-flow-control>`: how a fast producer is kept from burying a slow consumer.
