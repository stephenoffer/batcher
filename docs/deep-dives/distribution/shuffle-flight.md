...
- {doc}`Credit-based flow control </deep-dives/distribution/credit-flow-control>`: what stops a mapper flooding a reducer.
- {doc}`Distributed scheduling </deep-dives/distribution/distributed-scheduling>`: who runs where.
- {doc}`Mergeable algebra </deep-dives/operators/mergeable-algebra>`: why a bucket can be reduced independently.
