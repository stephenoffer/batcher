...
{doc}`Cost model </deep-dives/adaptive/cost-model>`.
...
- {doc}`Cardinality estimation </deep-dives/adaptive/cardinality-estimation>`: the biggest consumer.
- {doc}`Cost model </deep-dives/adaptive/cost-model>`: coefficient calibration.
- {doc}`Adaptive re-optimization </deep-dives/adaptive/adaptive-reoptimization>`: the within-query half of the loop.
- {doc}`The buffer pool </deep-dives/memory/buffer-pool>`: what the learned memory model sizes.
