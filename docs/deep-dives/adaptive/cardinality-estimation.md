...
{doc}`Adaptive re-optimization </deep-dives/adaptive/adaptive-reoptimization>`.
...
- {doc}`Reading a plan </user-guide/operate/explain-plans>`: the `est≈` and provenance tags in the tree.
...
- {doc}`Cost model </deep-dives/adaptive/cost-model>`: what consumes these row counts.
- {doc}`Adaptive re-optimization </deep-dives/adaptive/adaptive-reoptimization>`: measuring the truth at a breaker.
- {doc}`Learned metadata </deep-dives/adaptive/learned-metadata>`: where the NDVs and corrections are stored.
