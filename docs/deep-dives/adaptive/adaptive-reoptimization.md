...
- {doc}`Reading a plan </user-guide/operate/explain-plans>`: the `analyze=True` output above.
- {doc}`TPC-H benchmarks </benchmarks/tpch>`: the join shapes where re-planning pays.
- {doc}`Cardinality estimation </deep-dives/adaptive/cardinality-estimation>`: where the estimate under test comes from.
- {doc}`Learned metadata </deep-dives/adaptive/learned-metadata>`: the across-query half of the loop.
- {doc}`Cost model </deep-dives/adaptive/cost-model>`: what a corrected cardinality feeds into.
