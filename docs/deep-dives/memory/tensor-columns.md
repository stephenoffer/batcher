...
- {doc}`Arrow and memory </deep-dives/memory/arrow-memory>`: what a `FixedSizeList` buffer actually is.
- {doc}`GPU execution </deep-dives/distribution/gpu-execution>`: what consumes these tensors.
- {doc}`Expression evaluation </deep-dives/query/expression-evaluation>`: where the decode kernels run.
