...
- {doc}`Performance </user-guide/operate/performance>`: writing predicates that land on this tier.
- {doc}`Analytics benchmarks </benchmarks/analytics>`: the `filter → count` and `filter → project` ratios quoted above.
- {doc}`Expression evaluation </deep-dives/query/expression-evaluation>`: the Tier-0 oracle it must match.
- {doc}`Morsel parallelism </deep-dives/operators/morsel-parallelism>`: the loop the compiled artifact runs inside.
- {doc}`Cost model </deep-dives/adaptive/cost-model>`: how `jit_speedup` prices a compilable expression.
