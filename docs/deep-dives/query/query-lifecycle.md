...
- {doc}`Reading a plan </user-guide/operate/explain-plans>`: how to read the `explain()` output above.
- {doc}`Performance </user-guide/operate/performance>`: applying all of this to a slow query.
- {doc}`Analytics benchmarks </benchmarks/analytics>`: where the 0.5 ms fixed-overhead figure comes from.
- {doc}`Plan IR </deep-dives/query/plan-ir>`: the JSON wire contract the boundary speaks.
- {doc}`Morsel parallelism </deep-dives/operators/morsel-parallelism>`: how a plan becomes work for N cores.
- {doc}`Adaptive re-optimization </deep-dives/adaptive/adaptive-reoptimization>`: why steps 2 through 5 can run more than once.
