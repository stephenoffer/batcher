...
That is the cost of the interpreter, and it is the cost the {doc}`JIT </deep-dives/query/jit-compilation>` exists
to remove.
...
- {doc}`Expressions </user-guide/transform/expressions>`: the Python surface that builds these trees.
...
- {doc}`JIT compilation </deep-dives/query/jit-compilation>`: the Tier-1 path and what it can and cannot compile.
- {doc}`Plan IR </deep-dives/query/plan-ir>`: how an `Expr` gets here from Python.
- {doc}`Tensor columns </deep-dives/memory/tensor-columns>`: what the media decode kernels produce.
