...
- {doc}`Reading a plan </user-guide/operate/explain-plans>`: the same tree, rendered for humans.
- {doc}`Performance </user-guide/operate/performance>`: what to do when the plan is not the one you wanted.
- {doc}`TPC-H benchmarks </benchmarks/tpch>`: the query shapes these hints are tuned against.
- {doc}`Query lifecycle </deep-dives/query/query-lifecycle>`: where the document is produced and consumed.
- {doc}`Expression evaluation </deep-dives/query/expression-evaluation>`: what the engine does with an `Expr`.
- {doc}`Join algorithms </deep-dives/operators/join-algorithms>`: what the `strategy` hint actually selects.
