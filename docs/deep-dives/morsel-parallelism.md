...
A query has to be cut into pieces before N cores can work on it. Cut it into one piece per
core and a single slow partition stalls everyone; cut it too fine and scheduling overhead
eats the gain; cut it by row count alone and a table of 5 MB images blows out memory. The
unit that survives all three failures is the **morsel**.

A morsel is an Arrow `RecordBatch`; `bc_arrow::Morsel` is a type alias, not a wrapper, so
there is no second data structure. Its target size is 16,384 rows **or** 1 MiB, whichever it
hits first (`bc_arrow::MorselTarget`). The row bound keeps a narrow batch cache-resident;
the byte bound keeps a wide one from ballooning.
...
Sources do not produce well-sized batches. A Parquet reader emits row groups; a streaming
reader emits whatever arrived; a selective upstream filter emits crumbs. `morselize`
(`crates/bc-interp/src/ops/morsel.rs`) corrects both directions:

- **Too large**: split into row- and byte-bounded pieces.
- **Too small**: coalesce a run of undersized batches up to the target *before* splitting.

A batch that already fills a morsel is neither buffered nor copied. It splits or passes
through, both zero-copy, so well-sized input pays nothing.

:::{warning}
Skipping the coalesce step is expensive in a way that is easy to miss. Each tiny batch becomes
its own task and its own partial aggregate state to merge, so the per-morsel overhead is paid
in full for a fraction of the work. A high-cardinality group-by fed 256-row batches measured
**~19x slower** than the same rows in morsel-sized batches.
:::
...
widths cannot move the boundaries. Only when the average row approaches the byte budget does
...
count and is therefore not stable across machines. These are unordered relations; the tests
...
width, and the hash-shuffle bucket count keys off it. When it is `0` ("all cores"), the width
is `available_parallelism()` **capped by the number of morsels the inputs can produce**
(`par::auto_width`).

The cap matters because an idle worker is not free: rayon still wakes it, it contends for the
job queue, and (since pools are cached per width) a one-row query would otherwise install
and spin up a 96-thread pool. The engine's low-fixed-overhead goal is exactly this case. The
cap is an upper bound on *useful* parallelism at the leaves, so it can never remove
parallelism a plan could have used, and it never changes a result.
...
There is one deliberate exception. A plan containing a media decode (`Expr::contains_media_decode`)
lifts the cap to all cores. Decode does heavy, embarrassingly-parallel work *inside* a morsel,
...
is a real cost on the small and streaming paths (under streaming it is a new pool per
micro-batch), and sharing one pool per width also bounds the total worker-thread count when
several queries run at once.

## The allocator is part of the story

This is not a footnote. Every morsel-parallel operator allocates its output buffers per
...
:::{warning}
Measured on a 6M-row filter: 21.4 ms sequential; parallel wall time bottomed out at 4.0 ms
(5.3x on 96 cores) and then *regressed* past 32 workers. Adding cores made it slower. With
mimalloc's per-thread heaps, which recycle pages instead of returning them, the same filter
scales to 1.46 ms: 15x, no regression. `bc-py` installs it as the `#[global_allocator]`
because it is the cdylib every crate links into (`crates/bc-py/src/lib.rs`).
:::
...
shipped to Rust inside `EngineConfig` so the two sides cannot disagree about them.

## What it costs, and where it stops paying

Morsel granularity buys load balance and cache residency, and it costs a per-morsel
scheduling and state-merge overhead that only amortizes if the morsel is big enough. 16,384
rows was chosen to sit in L2/L3 for narrow data; the 19x regression above is what happens
when something upstream defeats it.

The honest headline number: **single-node parallelism reaches only about 1.7–3.8x on 16
cores** on the TPC-H shapes (`benchmarks/BENCHMARK_RESULTS.md`). The scan-and-aggregate core
scales well; the join does not, and the reason is Amdahl rather than mystery. Serial prefixes
...
- [Scaling benchmarks](../benchmarks/scaling.md): where the 1.7–3.8x figure comes from
