...
:::{seealso}
- {doc}`SQL to DataFrame </tutorials/foundations/sql-to-dataframe>`: the same query, both ways.
...
- {doc}`/tutorials/paths/ml-engineer`: the path onward, once a model needs to run in production.
