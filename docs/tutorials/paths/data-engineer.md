...
1. {doc}`Your first pipeline </tutorials/foundations/first-pipeline>`: the end-to-end flow.
...
- {doc}`Building a lakehouse </tutorials/pipelines/building-a-lakehouse>`: the same pieces, end to end.
...
- {doc}`/tutorials/paths/platform-engineer`: the operational half, once the pipelines exist.
