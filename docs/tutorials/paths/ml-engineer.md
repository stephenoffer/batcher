...
1. {doc}`Your first pipeline </tutorials/foundations/first-pipeline>`: the data flow a model
   plugs into.
1. {doc}`Batch inference </tutorials/ml/batch-inference>`: the `.ml.map_batches`
   pattern.
...
- {doc}`/tutorials/paths/data-scientist`: the analysis path that feeds this one.
- {doc}`/tutorials/paths/platform-engineer`: sizing, scheduling, and observability for the jobs you build.
