...
This page lists the four namespaces a {py:obj}`Dataset <batcher.Dataset>` carries for work the relational verbs have no spelling for: machine learning, data quality, slowly changing dimensions, and metadata. The dataset's own methods are on {doc}`dataset-transforms` and {doc}`dataset-terminal`.
...
- {doc}`dataset-transforms`: the `Dataset` methods these namespaces hang off.
- {doc}`groupby`: the `GroupBy` an aggregation returns.
