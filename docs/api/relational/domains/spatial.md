...
{doc}`/user-guide/analyze/domains/robotics`.
...
{doc}`/api/relational/domains/geospatial` instead.
...
- {doc}`/user-guide/analyze/domains/robotics` teaches these against a worked example.
- {doc}`/api/relational/domains/geospatial` for geometry on the Earth's surface.
