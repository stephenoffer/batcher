...
{doc}`/user-guide/analyze/domains/geospatial`.
...
- {doc}`/user-guide/analyze/domains/geospatial`: the guide that teaches these.
- {doc}`/api/relational/functions`: the non-spatial scalar, aggregate and window functions.
- {doc}`/cookbook/analytics/aggregates/geospatial-binning`: snapping coordinates to a grid by hand,
