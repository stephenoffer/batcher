...
Breadth on {py:class}`Expr <batcher.plan.expr_ir.core.Expr>` lives on accessor namespaces
rather than on the expression itself. Two hundred-odd string functions sit behind
{py:class}`.str <batcher.plan.expr_ir.namespaces.strings._StrNamespace>`, not on every
expression you build.

This page is the completeness index: one row per namespace, naming every method it carries,
so you can scan for a capability without knowing what it is called. For a signature, an
argument list, or a link to one method's own page, read {doc}`/api/accessors/index`, which
gives each namespace a page of grouped tables. The `Expr` methods these hang off are in
...
| `.mime_type()` | what the leading bytes say the payload is (`image/png`, `video/mp4`, `application/pdf`, ...), null when unrecognized, and the way to identify bytes that never came from a file read, such as a `ds.ml.download` or a blob column |
...
| {py:meth}`.extract_all(pattern) <batcher.plan.expr_ir.namespaces.strings._StrNamespace.extract_all>` | every regex match as a `List<Utf8>` (DuckDB `regexp_extract_all`) |
...
| {py:meth}`.escape_regex() <batcher.plan.expr_ir.namespaces.strings._StrNamespace.escape_regex>` | escape the regex metacharacters, so a value can be embedded in a pattern as a literal (DuckDB `regexp_escape`) |
| `.url_decode(form=True, malformed="null")` | Spark `try_url_decode`: `+` reads as a space and a `%` without two hex digits nulls the row, where the default leaves both as written |
| {py:meth}`.find_in_set(needle) <batcher.plan.expr_ir.namespaces.strings._StrNamespace.find_in_set>` | the 1-based position of a constant `needle` in a comma-separated list, or 0 (Spark `find_in_set`, → Int64) |
| {py:meth}`.parse_url(part, key=None) <batcher.plan.expr_ir.namespaces.strings._StrNamespace.parse_url>` | one component of a URL (`PROTOCOL`, `HOST`, `PATH`, `QUERY`, `REF`, `FILE`, `AUTHORITY`, `USERINFO`), or one query parameter with `key`; null when absent (Spark `parse_url`) |
...
{py:meth}`.century() <batcher.plan.expr_ir.namespaces.temporal._DtNamespace.century>`, {py:meth}`.decade() <batcher.plan.expr_ir.namespaces.temporal._DtNamespace.decade>`, {py:meth}`.weekday() <batcher.plan.expr_ir.namespaces.temporal._DtNamespace.weekday>` (ISO day of week, Monday = 1), {py:meth}`.last_day() <batcher.plan.expr_ir.namespaces.temporal._DtNamespace.last_day>` (last day
...
{py:meth}`.months_between(other, round_off=True) <batcher.plan.expr_ir.namespaces.temporal._DtNamespace.months_between>` is Spark's fractional month difference (→ Float64): whole months when both values share a day of the month or both fall on a month end, otherwise the leftover days and time over a fixed 31-day month, rounded to 8 places unless `round_off=False`. {py:meth}`.next_day(day_of_week) <batcher.plan.expr_ir.namespaces.temporal._DtNamespace.next_day>` is the first date strictly after the value on that weekday (Spark `next_day`, → Date). Spark `unix_date` is {py:func}`bt.partition_days <batcher.partition_days>`.

### More `.json` methods

These read a typed value at a JSON path. The last few describe the document's shape
instead, for when you don't yet know it.
...
Curating a training corpus, scrubbing PII and budgeting context windows are per-row scans.
They belong in the engine. Everything below scores a whole corpus in one vectorized pass,
and everything below is reachable from SQL under the naming rule the last section of this
page gives.

### Corpus quality heuristics

The character-class ratios and shape statistics on {py:class}`.str <batcher.plan.expr_ir.namespaces.strings._StrNamespace>` that Gopher, C4
and RefinedWeb-style filters threshold on to drop boilerplate and machine-generated text:
{py:meth}`.alpha_ratio() <batcher.plan.expr_ir.namespaces.strings._StrNamespace.alpha_ratio>`, {py:meth}`.digit_ratio() <batcher.plan.expr_ir.namespaces.strings._StrNamespace.digit_ratio>`, {py:meth}`.uppercase_ratio() <batcher.plan.expr_ir.namespaces.strings._StrNamespace.uppercase_ratio>`,
...
### Cleaning and PII scrubbing

{py:meth}`.remove_urls() <batcher.plan.expr_ir.namespaces.strings._StrNamespace.remove_urls>`, {py:meth}`.remove_emails() <batcher.plan.expr_ir.namespaces.strings._StrNamespace.remove_emails>`,
...
{py:meth}`.left(n) <batcher.plan.expr_ir.namespaces.strings._StrNamespace.left>`, a hard character cap, and {py:meth}`.truncate_words(n) <batcher.plan.expr_ir.namespaces.strings._StrNamespace.truncate_words>`, which never cuts mid-word.

### Detection predicates

Boolean, for a `filter`. {py:meth}`.has_url() <batcher.plan.expr_ir.namespaces.strings._StrNamespace.has_url>`, {py:meth}`.has_email() <batcher.plan.expr_ir.namespaces.strings._StrNamespace.has_email>`,
...
Each returns a `List<Utf8>`. {py:meth}`.extract_urls() <batcher.plan.expr_ir.namespaces.strings._StrNamespace.extract_urls>`, {py:meth}`.extract_emails() <batcher.plan.expr_ir.namespaces.strings._StrNamespace.extract_emails>`,
...
For dedup keys and prose corpora. {py:meth}`.slugify() <batcher.plan.expr_ir.namespaces.strings._StrNamespace.slugify>`, {py:meth}`.remove_bullets() <batcher.plan.expr_ir.namespaces.strings._StrNamespace.remove_bullets>`,
...
{py:meth}`.estimate_tokens(chars_per_token=4.0) <batcher.plan.expr_ir.namespaces.strings._StrNamespace.estimate_tokens>` and
...
On {py:class}`.list <batcher.plan.expr_ir.namespaces.collections._ListNamespace>`: {py:meth}`.len() <batcher.plan.expr_ir.namespaces.collections._ListNamespace.len>` (the embedding dimension),
{py:meth}`.is_zero_vector() <batcher.plan.expr_ir.namespaces.collections._ListNamespace.is_zero_vector>` (the failed-encoder check),
{py:meth}`.sum_squares() <batcher.plan.expr_ir.namespaces.collections._ListNamespace.sum_squares>`, {py:meth}`.l2_norm() <batcher.plan.expr_ir.namespaces.collections._ListNamespace.l2_norm>`,
...
held), {py:meth}`.mean() <batcher.plan.expr_ir.namespaces.collections._ListNamespace.mean>` / {py:meth}`.max() <batcher.plan.expr_ir.namespaces.collections._ListNamespace.max>` (mean and max pooling),
...
These sit on the dataset rather than a column. {py:meth}`ds.shuffle(seed=) <batcher.Dataset.shuffle>`,
{py:meth}`ds.stratified_split(label, test_size) <batcher.Dataset.stratified_split>`
(preserves each class's proportion, and value-hashed, so it is identical distributed),
{py:meth}`ds.sample_per_group(by, n) <batcher.Dataset.sample_per_group>`,
{py:meth}`ds.class_balance(label) <batcher.Dataset.class_balance>` and
{py:meth}`ds.class_weights(label) <batcher.Dataset.class_weights>`.

## Calling an accessor from SQL

Every method on this page is reachable from SQL. The name is the namespace and the method,
joined by an underscore: `col("s").str.slugify()` is `str_slugify(s)`, `col("i").image.phash()` is `image_phash(i)`,
`col("t").dt.quarter_end()` is `dt_quarter_end(t)`. The first argument is the value the
accessor hangs off. The rest are the method's own arguments, in order.
