...
| `.video` | `decode()` (header-only struct `{width, height, num_frames, duration_secs, fps}`), `frames(num_frames, width, height)` (evenly-spaced frames as one fixed-shape `(num_frames, height, width, 3)` uint8 tensor — the video training-ingest kernel), `thumbnail(max_size)` (the clip's middle frame as PNG bytes, aspect preserved), `frame_at(second, max_size)` (the frame shown at `second`, as PNG bytes) |
