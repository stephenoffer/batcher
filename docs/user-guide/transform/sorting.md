...
- {doc}`Filtering </user-guide/transform/filtering>`: cut rows before you order them.
...
- {doc}`Performance </user-guide/operate/performance>`: the spill path and the memory budget.
- {doc}`Sort internals </deep-dives/operators/sort-internals>`: the run generation and k-way merge
...
- {doc}`Top k per group </cookbook/analytics/top-k-per-group>`: the window recipe, worked
  end to end.
