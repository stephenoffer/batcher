...
Read {doc}`/user-guide/transform/expressions` and {doc}`/user-guide/transform/expression-accessors` first. Every example here
...
- {doc}`/user-guide/transform/expressions`: the core language these recipes are built from.
- {doc}`/user-guide/transform/expression-accessors`: the `.str`, `.dt`, `.list`, `.struct`, and `.json` methods.
