...
`.json` accessors (see {doc}`Expressions </user-guide/transform/expressions>`) in a `select`: `.struct.field(name)`
...
- {doc}`Filtering </user-guide/transform/filtering>`: row selection, deduplication, limits.
