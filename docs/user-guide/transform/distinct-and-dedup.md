...
- {doc}`Sorting </user-guide/transform/sorting>`: the tie-order rules that decide which row `keep="any"` gives you.
- {doc}`Data quality </user-guide/trust/data-quality>`: assert uniqueness instead of silently repairing it.
- {doc}`Aggregation internals </deep-dives/operators/aggregation-internals>`: the canonical hash key
  every dedup path shares, and why it has to be shared.
- {doc}`Deduplication recipe </cookbook/data-engineering/deduplication>`: the same three
  jobs, on a real table.
- {doc}`Training-data dedup </cookbook/ml/pipelines/training-data-dedup>`: near-duplicate removal
