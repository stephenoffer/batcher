...
These pages cover `Expr`, the language every column computation is written in. An expression
...
| {doc}`Expression accessors <expression-accessors>` | The `.str`, `.dt`, `.list`, `.struct`, and `.json` namespaces, one per column kind |
...
- {doc}`/cookbook/expressions/index`: 34 runnable pages of the same language.
