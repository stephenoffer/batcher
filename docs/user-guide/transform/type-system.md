...
across partitions. See {doc}`distinct and dedup </user-guide/transform/distinct-and-dedup>`.
...
- {doc}`Expressions </user-guide/transform/expressions>`: the full method surface, per type.
...
- {doc}`Data quality </user-guide/trust/data-quality>`: assert the types and ranges you expect, instead of
  discovering them.
- {doc}`Arrow memory </deep-dives/memory/arrow-memory>`: the zero-copy boundary the
  normalization happens at, and why it happens exactly once.
- {doc}`Expression evaluation </deep-dives/query/expression-evaluation>`: the two numeric paths
  the widening buys, and what the JIT does with them.
- {doc}`Tensor columns </deep-dives/memory/tensor-columns>`: fixed-shape tensors, the one nested
  type with a shape contract.
- {doc}`Schema evolution </cookbook/data-engineering/schema-evolution>`: types that change
