...
- {doc}`Filtering </user-guide/transform/filtering>`: predicates, which is how a stratum is defined.
- {doc}`Preprocessors </ml/preparing/preprocessors/index>`: fitting feature statistics on the train split
  only.
- {doc}`Cardinality estimation </deep-dives/adaptive/cardinality-estimation>`: the sketches behind
  `approx_n_unique`, and the error bounds they hold to.
- {doc}`Train/test split recipe </cookbook/ml/pipelines/train-test-split>`: the leak-free split on a
  real feature table.
- {doc}`A/B testing </cookbook/analytics/ab-testing>`: hash-bucketed assignment, the same
