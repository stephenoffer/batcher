...
- {doc}`Expression accessors </user-guide/transform/expression-accessors>`: the `.str`, `.dt`, `.list`,
  `.struct`, and `.json` namespaces, which hold the methods specific to one kind of
  column.
- {doc}`Expression recipes </user-guide/transform/expression-recipes>`: porting from pandas or Polars, feature
...
- {doc}`Transformations </user-guide/transform/transformations>`: where expressions are applied to a Dataset.

And for what happens to an expression after you write it:

- {doc}`Expression evaluation </deep-dives/query/expression-evaluation>`: how a tree of `Expr`
  nodes becomes vectorized work over an Arrow batch.
- {doc}`JIT compilation </deep-dives/query/jit-compilation>`: when the Cranelift tier compiles an
