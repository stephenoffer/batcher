...
The functions that take a key are covered in {doc}`governance`. This page is about where
that key comes from.
...
:::{seealso}
- {doc}`governance`: the masking, row filters, and audit trail these keys feed.
- {doc}`reading-data`: the connectors whose credentials take the same references.
