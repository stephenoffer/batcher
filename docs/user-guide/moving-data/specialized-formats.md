...
This page covers the {py:obj}`bt.read <batcher.read>` entry points for the formats that are not a file of
rows: chunked n-dimensional arrays, web crawls, PDFs, genomics files, LiDAR point clouds,
and robot and vehicle logs. Each one lands as an ordinary `Dataset`, so a LiDAR sweep and a Parquet table
join without either of them leaving Arrow.
...
They share one shape. `bt.read.<name>(path_or_uri, **opts)` hands back a lazy `Dataset`, and
nothing is fetched until a terminal op runs.
...
## Databases and warehouses are next door

A database is not a specialized format, and the readers that reach one are documented where
their setup is. {py:obj}`bt.read.sql <batcher.api.io_namespace.reader.Reader.sql>` addresses any
SQLAlchemy-style connection URI, and {doc}`/integrations/databases/databases` is the guide to it:
which backend serves which scheme, where credentials come from, what reaches the server, and how
to partition one extract into several parallel queries without making the database do N full
scans. {doc}`/integrations/warehouses/index` covers Snowflake, BigQuery, and Databricks;
{doc}`/integrations/databases/index` covers MongoDB, Elasticsearch, and the key-value stores;
{doc}`/integrations/streams/index` covers Kafka and the other message queues.
...
A reader that needs an optional extra or a remote store is shown but not executed here:

```python
# docs: skip
grids = bt.read.zarr("s3://bucket/array.zarr")
shared = bt.read.delta_sharing("config.share#share.schema.table")
crawl = bt.read.warc("s3://commoncrawl/crawl-data/CC-MAIN-2024-10/segments/*.warc.gz")
```
...
Five genomics formats read as ordinary tables, and each reports what its file says: BED coordinates stay 0-based half-open, GFF and VCF stay 1-based. The following table lists them:

| Reader | Rows | Writer |
| --- | --- | --- |
| `read.fasta` | One per record: `id`, `description`, `sequence` | `write.fasta` |
| `read.fastq` | One per read: `id`, `description`, `sequence`, `quality` | `write.fastq` |
| `read.bed` | One per interval, BED3 to BED12 by the file's width; a bedGraph reads as `chrom`, `start`, `end`, `value` | `write.bed` |
| `read.gff` | One per GFF3 or GTF feature, nine columns, attributes as text | `write.gff` |
| `read.vcf` | One per variant; `INFO`, `FORMAT` and each sample column as text | none |

Every reader takes the gzipped spelling of its suffixes, such as `.vcf.gz`, `.bed.gz` or `.fa.gz`, both as a single file and when listing a directory. A bgzipped VCF is ordinary multi-member gzip and reads the same way. Line endings may be `\n`, `\r\n`, or a bare `\r`.

A few format rules decide what a row is. A GFF3 file's `##FASTA` section ends the annotation table, so the appended sequences are not read as features. In FASTA, a line starting with `;` is a comment wherever it appears. A BED `track` or `browser` line is skipped, and a `track type=bedGraph` line, or a `.bedgraph` or `.bdg` suffix, makes the fourth column a float `value`. VCF sample names must be unique, and a sample whose name collides with a fixed column, such as one called `pos`, is renamed `sample_pos`.
...
vcf = (
    ...
    "chr1\t100\trs1\tA\tG\t50\tPASS\tAF=0.25\tGT\t0/1\n"
)
with open(os.path.join(root, "calls.vcf.gz"), "wb") as fh:
    fh.write(gzip.compress(vcf.encode()))
print(bt.read.vcf(os.path.join(root, "calls.vcf.gz")).select("chrom", "pos", "NA1").to_pydict())
# {'chrom': ['chr1'], 'pos': [100], 'NA1': ['0/1']}

gff = "##gff-version 3\nchr1\tsrc\tgene\t1\t90\t.\t+\t.\tID=g1\n##FASTA\n>chr1\nACGT\n"
with open(os.path.join(root, "genes.gff3"), "w") as fh:
    fh.write(gff)
print(bt.read.gff(os.path.join(root, "genes.gff3")).select("type", "end").to_pydict())
# {'type': ['gene'], 'end': [90]}
```

BCF, VCF's binary encoding, is not supported. A `.bcf` file, or BGZF data under a name that does not end in `.gz`, is refused with an error that says so. Convert it with `bcftools view -Ov`. There is no VCF writer, because a VCF's `##` block declares every `INFO` and `FORMAT` key and a table does not carry that block.

Parallelism comes from files. Every genomics format splits per file, since a record boundary cannot be found from a byte offset, so a distributed read runs one task per file and a single large file reads on one worker. {doc}`/cookbook/expressions/genomics/files` covers the read and write options in depth.
...
- {doc}`/integrations/databases/databases`: reading a SQL database, including parallel extraction.
- {doc}`/user-guide/analyze/domains/robotics`: what to do with an MCAP or MDF4 log once it is a table.
- {doc}`/api/symbols/readers-and-writers`: every reader and writer, with signatures.
