...
watermarks are on {doc}`index`, because they are the mechanism these lean on rather than
an operator.
...
A session differs from the fixed windows on {doc}`index` in one way, and it is a difference in shape over time:
...
from [`benchmarks/scenarios/streaming/state_checkpoint.py`](https://github.com/stephenoffer/batcher/blob/main/benchmarks/scenarios/streaming/state_checkpoint.py). Run it to reproduce them. They
...
- {doc}`index`: sources, sinks, triggers, output modes, windows, and checkpoints.
- {doc}`monitoring`: the state each of these operators reports per micro-batch.
- {doc}`/cookbook/streaming/stream-join`: the interval join end to end.
- {doc}`emission`: which of these shapes emit per batch.
