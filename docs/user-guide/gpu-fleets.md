...
- {doc}`../ml/gpu`: choosing devices and batch sizes for a model, from the pipeline side.
- {doc}`governance`: the row and column half of the same policy layer.
- {doc}`../configuration/options`: every accelerator field with its default and unit.
- {doc}`../api/governance`: the residency reference.
- {doc}`../deep-dives/distributed-scheduling`: how a stage becomes tasks on a cluster.
- {doc}`observability`: the metrics sink the `energy.*` rows land in.
