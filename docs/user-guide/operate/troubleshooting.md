...
| The answer is wrong | {doc}`/user-guide/operate/explain-plans`, then {doc}`/user-guide/trust/data-quality` to assert what you expected |
| Correct, but too slow | {doc}`/user-guide/operate/performance`, then {doc}`/tutorials/optimizing-a-slow-query` |
| A query is taking far too long and you want it to stop | "Stopping a query that is taking too long", below |
| Out of memory | "A large query runs out of memory", below, and {doc}`/deep-dives/memory/spilling` |
| The same query keeps recomputing | {doc}`/user-guide/operate/caching` |
| A file will not read, or the schema disagrees across files | {doc}`/user-guide/moving-data/reading-data` and {doc}`/user-guide/transform/type-system` |
...
| You want to know what the engine actually did | {doc}`/user-guide/operate/observability` |
...
- {doc}`Explain plans </user-guide/operate/explain-plans>`: the first thing to read when the answer or the
  runtime surprises you, and how to tell whether a predicate reached the scan.
- {doc}`Performance and memory </user-guide/operate/performance>`: caching, spill tuning, reading a query
  plan.
- {doc}`Exceptions API </api/operations/exceptions>`: the reference for every type in the table above,
...
- {doc}`Observability </user-guide/operate/observability>`: the event stream, the progress reporter, and
  `bt.start_ui()` for watching a running query.
- {doc}`Spilling </deep-dives/memory/spilling>`: what `spill=True` actually does, and why a spilled
