...
| A file will not read, or the schema disagrees across files | {doc}`/user-guide/moving-data/reading-data` and {doc}`/user-guide/transform/columns/type-system` |
...
- {doc}`Explain plans </user-guide/operate/tuning/explain-plans>`: the first thing to read when the answer or the
  runtime surprises you, and how to tell whether a predicate reached the scan.
- {doc}`Performance and memory </user-guide/operate/tuning/performance>`: caching, spill tuning, reading a query
  plan.
...
- {doc}`Unstable nodes </user-guide/operate/running/unstable-nodes>`: when the job is failing because
...
- {doc}`Agent skills </agents>`: `debug-a-batcher-query` is the triage tree a
