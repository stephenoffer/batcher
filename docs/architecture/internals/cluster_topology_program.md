...
Companion to {doc}`/architecture/internals/gpu_datacenter_program`, which made a device's own properties visible to the
...
The same five {doc}`/architecture/internals/gpu_datacenter_program` states, with two that carried most of the weight
here:
...
- {doc}`/architecture/internals/gpu_datacenter_program` — the device-level work this builds on.
