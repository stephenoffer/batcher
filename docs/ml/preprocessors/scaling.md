...
:::{seealso}
- {doc}`encoding`: the categorical half of the same job.
- {doc}`index`: the fit/transform contract and the full preprocessor table.
- {doc}`../statistics-and-drift`: the statistics that tell you which transform a column needs.
:::
