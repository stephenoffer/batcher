...
Read {doc}`llm` first for how the generation itself runs.
...
:::{seealso}
- {doc}`llm`: running the generation these outputs come from.
- {doc}`llm-evaluation`: scoring the parsed results.
- {doc}`../user-guide/expression-accessors`: the `.json` accessor these methods build on.
:::
