...
feeds) see the {doc}`lakehouse guide </user-guide/moving-data/lakehouse>`.
...
friends; see {doc}`cloud storage </user-guide/moving-data/cloud-storage>`). Delta also accepts
...
- {doc}`Iceberg </integrations/lakehouse/iceberg>` and {doc}`Hudi </integrations/lakehouse/hudi>`: the other two table formats.
