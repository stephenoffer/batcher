...
- {doc}`/getting-started/migration/index`: the mapping tables the migration skills build on.
- {doc}`/architecture/internals/extending`: the contributor cookbook behind the `add-*` skills.
- {doc}`/user-guide/index`: the task-oriented guides the usage skills point into.
- {doc}`/user-guide/operate/running/troubleshooting`: the human-facing companion to
