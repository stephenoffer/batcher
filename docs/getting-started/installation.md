...
{doc}`install/index` lists every supported platform, including musl-based Linux such as Alpine, and helps you choose between pip, uv, conda, containers, Ray clusters, and offline installs.
...
{py:func}`bt.versions() <batcher.versions>` returns a dict with the Batcher version, the compiled engine's version and build profile, and one key per optional backend. Check it first when an extra seems to be missing:
...
| `pandas`, `polars`, `numpy` | Converting results with `to_pandas`, `to_polars`, and `to_numpy` | {doc}`migration/index` |
...
- {doc}`quickstart`: run a complete pipeline in five minutes.
- {doc}`concepts/index`: the lazy, immutable dataset model that shapes the API.
