...
:link: /architecture/deep-dives/distribution/credit-flow-control
:link-type: doc
...
- {doc}`/getting-started/tutorials/paths/data-engineer`: the pipelines you are operating.
- {doc}`Optimizing a slow query </getting-started/tutorials/foundations/optimizing-a-slow-query>`: a real diagnosis, start to finish.
...
- {doc}`/architecture/internals/index`: how the engine works, when an incident needs it.
