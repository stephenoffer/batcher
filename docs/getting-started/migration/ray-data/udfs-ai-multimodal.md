...
| `Concatenator` | `batcher.ml.Concatenator` | mismatch | Differs: Ray defaults output\_column\_name='concat\_out', drops the input columns and emits an ndarray column (dtype=, flatten=, raise\_if\_missing=); Batcher defaults output\_column='features', keeps the inputs (drop=False) and emits a list column. Codemod: output\_column='concat\_out', drop=True. Wave W0. |
| `CountVectorizer` | `batcher.ml.CountVectorizer` | mismatch | Differs: Ray tokenizes by splitting on single spaces without lowercasing (tokenization\_fn=) over several columns and writes a count vector per column; Batcher follows scikit-learn (lowercase=True, token\_pattern of 2+ word chars, vocabulary order) on one column into a 'features' output. Codemod: lowercase=False, token\_pattern=r'\[\^ \]+'. Wave W0. |
