...
- {doc}`Quality gates </cookbook/data-engineering/quality-gates>`: the same idea, applied to values instead of columns.
- {doc}`Multi-source join </cookbook/data-engineering/multi-source-join>`: where a type mismatch does its real damage.
- {doc}`Incremental ingest </cookbook/data-engineering/incremental-ingest>`: the loader dropping new files into the
  directory that drifted.
- {doc}`Reading data </user-guide/moving-data/reading-data>`: the reader options in full.
- {doc}`Type system </user-guide/transform/type-system>`: the promotion rules behind the lattice.
