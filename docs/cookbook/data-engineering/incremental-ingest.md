...
{doc}`CDC pipeline </cookbook/data-engineering/cdc-pipeline>`).
...
- {doc}`Deduplication </cookbook/data-engineering/deduplication>`: the incoming batch has duplicates in it.
- {doc}`File compaction </cookbook/data-engineering/file-compaction>`: a thousand small arrivals make a thousand
  small files.
- {doc}`Quality gates </cookbook/data-engineering/quality-gates>`: what to check before the arrivals reach the table.
