...
- {doc}`Slowly changing dimensions </cookbook/data-engineering/modeling/slowly-changing-dimensions>`: keep the history
  instead of overwriting it.
- {doc}`Deduplication </cookbook/data-engineering/maintenance/deduplication>`: the same collapse-by-sequence trick, on a feed
  with no deletes.
