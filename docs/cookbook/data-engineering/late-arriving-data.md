...
See {doc}`partition backfill </cookbook/data-engineering/partition-backfill>` for the mechanics of the replace.
...
streaming aggregate*, not counted late and not counted at all. There is no configuration
...
- {doc}`Partition backfill </cookbook/data-engineering/partition-backfill>`: the atomic replace that `rebuild` leans on.
- {doc}`Deduplication </cookbook/data-engineering/deduplication>`: late rows are often duplicate rows too.
- {doc}`CDC pipeline </cookbook/data-engineering/cdc-pipeline>`: out-of-order changes, sequenced rather than windowed.
- {doc}`Streaming </user-guide/moving-data/streaming>`: triggers, output modes, and the streaming
  query handle.
