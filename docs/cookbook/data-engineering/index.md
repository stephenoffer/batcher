...
{doc}`/cookbook/data-engineering/etl-pipeline` is the whole arc in one page: raw records in, deduplicated and rolled
...
get the data in ({doc}`incremental ingest </cookbook/data-engineering/incremental-ingest>`), put a gate in front of
the write ({doc}`quality gates </cookbook/data-engineering/quality-gates>`), make the write idempotent
({doc}`deduplication </cookbook/data-engineering/deduplication>`), and only then worry about the table's shape over
time ({doc}`schema evolution </cookbook/data-engineering/schema-evolution>`, {doc}`file compaction </cookbook/data-engineering/file-compaction>`).
...
```{toctree}
:hidden:
:caption: Keeping the table correct
