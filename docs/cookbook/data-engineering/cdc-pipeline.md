...
- {doc}`Slowly changing dimensions </cookbook/data-engineering/slowly-changing-dimensions>`: keep the history
  instead of overwriting it.
- {doc}`Deduplication </cookbook/data-engineering/deduplication>`: the same collapse-by-sequence trick, on a feed
  with no deletes.
- {doc}`Late-arriving data </cookbook/data-engineering/late-arriving-data>`: the other reason yesterday's numbers move.
