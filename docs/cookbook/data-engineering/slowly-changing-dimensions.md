...
{doc}`deduplication </cookbook/data-engineering/deduplication>`).
...
- {doc}`CDC pipeline </cookbook/data-engineering/cdc-pipeline>`: when the source is a change feed, not a snapshot.
- {doc}`Multi-source join </cookbook/data-engineering/multi-source-join>`: joining facts to dimensions in general.
- {doc}`Deduplication </cookbook/data-engineering/deduplication>`: getting to one row per key before the load.
