...
{doc}`multi-source join </cookbook/data-engineering/multi-source-join>`, where that is the whole story.
...
- {doc}`Deduplication </cookbook/data-engineering/deduplication>`: what `unique` found, and what to do about it.
- {doc}`Schema evolution </cookbook/data-engineering/schema-evolution>`: gating the shape rather than the values.
- {doc}`Multi-source join </cookbook/data-engineering/multi-source-join>`: the orphan key, and what an inner join does
  with it.
