...
`bt.compact` (see {doc}`file compaction </cookbook/data-engineering/file-compaction>`) cleans up after a write that did
not.
...
- {doc}`Late-arriving data </cookbook/data-engineering/late-arriving-data>`: why the day needed rebuilding in the
  first place.
- {doc}`Slowly changing dimensions </cookbook/data-engineering/slowly-changing-dimensions>`: when history is the
  point, not the obstacle.
- {doc}`File compaction </cookbook/data-engineering/file-compaction>`: cleaning up the parts a partial rewrite leaves.
