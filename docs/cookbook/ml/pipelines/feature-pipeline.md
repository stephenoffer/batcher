...
comparison you were about to make. See {doc}`train/test split </cookbook/ml/pipelines/train-test-split>` for
...
- {doc}`Train/test split </cookbook/ml/pipelines/train-test-split>`: hash splits, group leakage, and time-based cuts.
- {doc}`Recommender features </cookbook/ml/pipelines/recommender-features>`: aggregate and window features over an
  event log.
...
- {doc}`Tensor columns </deep-dives/memory/tensor-columns>`: why a feature vector travels with its
  shape.
