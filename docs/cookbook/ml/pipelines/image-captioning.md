...
- {doc}`Image classification </cookbook/ml/pipelines/image-classification>`: the discriminative version of this pipeline.
- {doc}`Audio transcription </cookbook/ml/pipelines/audio-transcription>`: the same fetch → decode → model shape, for
  sound.
...
- {doc}`Multimodal-ingest benchmarks </benchmarks/multimodal-ingest>` and
  ...
  quoted here.
- {doc}`GPU execution </deep-dives/distribution/gpu-execution>`: how the decode stage and the model stage
  overlap.
