...
- {doc}`Image captioning </cookbook/ml/pipelines/image-captioning>`: the same pipeline with a vision-language model.
- {doc}`Audio transcription </cookbook/ml/pipelines/audio-transcription>`: the same decode → model shape, for sound.
...
- {doc}`GPU execution </deep-dives/distribution/gpu-execution>`: how the CPU and GPU stages overlap.
