...
- {doc}`AI and GPU benchmarks </benchmarks/results/ai-and-gpu>`: the LLM batch-inference
...
- {doc}`HuggingFace integration </integrations/compute/huggingface>`: model ids, tokenizers, and
  datasets.
- {doc}`GPU execution </architecture/deep-dives/distribution/gpu-execution>`: why the pool is warm and the stages
  overlap.
