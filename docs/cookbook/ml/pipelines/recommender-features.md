...
{doc}`train/test split </cookbook/ml/pipelines/train-test-split>` for the other two ways that goes wrong.

## See also

- {doc}`Feature pipeline </cookbook/ml/pipelines/feature-pipeline>`: scale, encode, and assemble these columns.
- {doc}`Train/test split </cookbook/ml/pipelines/train-test-split>`: entity splits, time splits, and leakage.
- {doc}`Window functions </user-guide/analyze/window-functions>`: frames, ranks, and lag/lead in full.
- {doc}`Aggregations </user-guide/analyze/aggregations>`: the mergeable aggregate surface.
...
- {doc}`Mergeable algebra </deep-dives/operators/mergeable-algebra>`: why these aggregates give the
  same answer on one core and on a cluster.
- {doc}`Sessionization </cookbook/analytics/sessionization>`: the window-function recipe this one borrows
