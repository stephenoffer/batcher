...
{doc}`preprocessor </cookbook/ml/pipelines/feature-pipeline>` on `train`, transform both parts, and the fitted
...
{doc}`Training-data dedup </cookbook/ml/pipelines/training-data-dedup>` covers the fuzzy case, which is the one that
matters on a real corpus.

## See also

- {doc}`Feature pipeline </cookbook/ml/pipelines/feature-pipeline>`: fit on train, transform both parts.
- {doc}`Training-data dedup </cookbook/ml/pipelines/training-data-dedup>`: remove the twins before splitting.
- {doc}`Recommender features </cookbook/ml/pipelines/recommender-features>`: the point-in-time filter that is the
  time-split's cousin.
...
- {doc}`Sampling </user-guide/transform/sampling>`: the sampling surface the split is built on.
