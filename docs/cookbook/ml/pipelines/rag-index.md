...
- {doc}`Text embeddings </cookbook/ml/pipelines/text-embeddings>`: the encoder stage in detail.
- {doc}`LLM batch scoring </cookbook/ml/pipelines/llm-batch-scoring>`: engines, prompt templates, typed outputs.
...
- {doc}`Tensor columns </deep-dives/memory/tensor-columns>`: how the vectors are stored and shipped.
