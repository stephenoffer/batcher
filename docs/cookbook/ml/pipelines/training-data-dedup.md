...
- {doc}`Train/test split </cookbook/ml/pipelines/train-test-split>`: dedup first, then split, in that order.
- {doc}`Text embeddings </cookbook/ml/pipelines/text-embeddings>`: the embedding half of `similarity_join`.
...
- {doc}`Distinct and dedup </user-guide/transform/distinct-and-dedup>`: the exact-match surface, and
  when it is enough.
- {doc}`Deduplication </cookbook/data-engineering/deduplication>`: the same problem on an event stream,
  where the key is known.
- {doc}`ML API reference </api/models/ml>`: `near_duplicates`, `drop_near_duplicates`,
  `similarity_join`.
- {doc}`Join algorithms </deep-dives/operators/join-algorithms>`: the equi-join the LSH banding
