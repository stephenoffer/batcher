...
- {doc}`RAG index </cookbook/ml/pipelines/rag-index>`: chunk long documents before embedding them.
- {doc}`Training-data dedup </cookbook/ml/pipelines/training-data-dedup>`: the near-duplicate pass to run before you
...
- {doc}`Tensor columns </deep-dives/memory/tensor-columns>`: how a vector column is laid out.
