...
{doc}`preprocessor </cookbook/ml/pipelines/features/feature-pipeline>` on `train`, transform both parts, and the fitted
...
{doc}`Training-data dedup </cookbook/ml/pipelines/features/training-data-dedup>` covers the fuzzy case, which is the one that
...
- {doc}`Recommender features </cookbook/ml/pipelines/features/recommender-features>`: the point-in-time filter that is the
  time-split's cousin.
