...
- {doc}`Feature pipeline </cookbook/ml/pipelines/features/feature-pipeline>`: scale, encode, and assemble these columns.
- {doc}`Train/test split </cookbook/ml/pipelines/features/train-test-split>`: entity splits, time splits, and leakage.
