# Categorical encoding

One-hot is fine for a handful of categories and catastrophic for a million user ids. The alternatives trade information for width: ordinal keeps one column, frequency and target encoding keep one column carrying signal, and hashing bounds the width outright.

The script runs every encoder over the same city column, including `OrdinalEncoder` on an unseen category, a smoothed `TargetEncoder`, and `RareCategoryEncoder`, which folds the long tail into one bucket before you encode.

The whole script, executed on every test run:

```{literalinclude} ../../../../examples/ml/preprocessing_encoding.py
:language: python
:linenos:
```

Run it yourself:

```bash
python examples/ml/preprocessing_encoding.py
```

## See also

- {doc}`/cookbook/ml/preprocessing/preprocessing_chain`: chaining preprocessors into one fitted pipeline.
- {doc}`/cookbook/ml/preprocessing/preprocessing_imputation`: filling missing values, and keeping the fact that they were missing.
- {doc}`/ml/index`: the ML surface these recipes sit on.
- {doc}`/ml/preparing/preprocessors/index`: the fit and transform steps most pipelines start with.
