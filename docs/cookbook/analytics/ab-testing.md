...
- {doc}`Funnel analysis </cookbook/analytics/funnel-analysis>`: the one-row-per-user collapse, in more detail.
- {doc}`Retention curves </cookbook/analytics/retention-curves>`: another denominator that has to come from the
  cohort rather than from the behavior log.
- {doc}`Sampling </user-guide/transform/sampling>`: `sample` is a stable seeded content hash, so a
...
- {doc}`Join algorithms </deep-dives/operators/join-algorithms>`: what the left join does with the
