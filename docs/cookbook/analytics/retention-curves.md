...
Same skeleton as {doc}`cohort analysis </cookbook/analytics/cohort-analysis>`, in days rather than months. The
...
- {doc}`Cohort analysis </cookbook/analytics/cohort-analysis>`: the same skeleton, measured in months and revenue.
- {doc}`A/B testing </cookbook/analytics/ab-testing>`: the other page where the denominator has to come from the
...
- {doc}`Aggregation internals </deep-dives/operators/aggregation-internals>`: why the HyperLogLog
