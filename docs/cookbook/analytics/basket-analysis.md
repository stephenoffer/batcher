...
- {doc}`Top k per group </cookbook/analytics/top-k-per-group>`: keep the best three partners per item rather than
...
- {doc}`Distinct and dedup </user-guide/transform/distinct-and-dedup>`: duplicate line items in a
  basket will inflate every count on this page, so dedupe `(order_id, item)` first.
- {doc}`Join algorithms </deep-dives/operators/join-algorithms>`: how the build side and the
