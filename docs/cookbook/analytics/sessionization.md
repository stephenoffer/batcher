...
- {doc}`Funnel analysis </cookbook/analytics/funnel-analysis>`: scope a funnel to one session by grouping on
  `(user, session)` instead of `user`.
- {doc}`Time-series rollups </cookbook/analytics/time-series-rollups>`: the other side of the clock, where the
...
- {doc}`Window internals </deep-dives/operators/window-internals>`: the partition-sort-scan the
