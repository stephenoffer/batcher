...
- {doc}`Basket analysis </cookbook/analytics/basket-analysis>`: ranking pairs by lift instead of rows by revenue.
- {doc}`Cohort analysis </cookbook/analytics/cohort-analysis>`: the other use of `partition_by` without a sort,
  where the window labels rather than ranks.
- {doc}`Sorting </user-guide/transform/sorting>`: what `sort` costs, and when it spills.
- {doc}`Window internals </deep-dives/operators/window-internals>`: why the partitioned sort beats
  the global one.
- {doc}`Sort internals </deep-dives/operators/sort-internals>`: the heap that `top_k` fuses into.
