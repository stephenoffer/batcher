...
- {doc}`Anomaly detection </cookbook/analytics/anomaly-detection>`: what to do once the series is dense.
- {doc}`Sessionization </cookbook/analytics/sessionization>`: the opposite problem, where the buckets have to be
...
- {doc}`Aggregation internals </deep-dives/operators/aggregation-internals>`: how the daily rollup
