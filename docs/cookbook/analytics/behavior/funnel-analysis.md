...
- {doc}`Sessionization </cookbook/analytics/behavior/sessionization>`: scoping the funnel to a single visit.
- {doc}`Retention curves </cookbook/analytics/behavior/retention-curves>`: the same one-row-per-user shape, over days.
- {doc}`Aggregations </user-guide/analyze/aggregations>`: `count_if` and the rest of `agg`.
- {doc}`Pivoting </user-guide/analyze/pivoting>`: `pivot` and `unpivot`, the two moves this page turns on.
- {doc}`Joins </user-guide/analyze/joins>`: what the self-join you are avoiding would have cost.
- {doc}`Join algorithms </architecture/deep-dives/operators/join-algorithms>`: why the intermediate blows up, in detail.
- {doc}`Dataset API </api/relational/dataset>`: `pivot`, `unpivot`, `group_by`.
