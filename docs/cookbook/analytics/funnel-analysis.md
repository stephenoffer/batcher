...
- {doc}`Sessionization </cookbook/analytics/sessionization>`: scoping the funnel to a single visit.
- {doc}`Retention curves </cookbook/analytics/retention-curves>`: the same one-row-per-user shape, over days.
...
- {doc}`Join algorithms </deep-dives/operators/join-algorithms>`: why the intermediate blows up, in detail.
