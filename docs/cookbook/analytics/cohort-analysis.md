...
- {doc}`Retention curves </cookbook/analytics/retention-curves>`: the same cohort skeleton, measured in days
  and normalized to a rate.
- {doc}`Funnel analysis </cookbook/analytics/funnel-analysis>`: the other one-row-per-user collapse, and the
...
- {doc}`Window internals </deep-dives/operators/window-internals>`: what the partition-by shuffle
