...
{doc}`time series rollups </cookbook/analytics/time-series-rollups>`. Otherwise a gap in the metrics quietly
stretches your one-hour window across a day.
:::

## See also

- {doc}`Time series rollups </cookbook/analytics/time-series-rollups>`: densify the series before you window over it.
- {doc}`A/B testing </cookbook/analytics/ab-testing>`: the other page here where a pooled statistic hides the
...
- {doc}`Aggregation internals </deep-dives/operators/aggregation-internals>`: why `median` is the
