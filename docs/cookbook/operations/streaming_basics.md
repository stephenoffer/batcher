...
Batcher runs batch and streaming on one operator set, so the transformation you tested on a file is the one that runs on the stream. The script builds its input with {py:obj}`bt.from_pydict <batcher.from_pydict>` so that it terminates, but the pipeline shape is the same for Kafka. Watch the last assertion. The answer collected in one go and the answer summed over `iter_batches` agree, and everything else on this page rests on that.
