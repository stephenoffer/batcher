...
That is rarely the binding constraint, because a sequencing run arrives as many files, one per lane or sample or mate, and a reference assembly as one file per chromosome. Where a single enormous file really is the input, convert it to Parquet once and read that.
