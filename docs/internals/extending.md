...
and `run-quality-gate`. See {doc}`../agents/index` for the full catalog, including the
usage and migration skills aimed at people building *with* Batcher rather than on it.


## See also

- {doc}`index`: the subsystem tour these recipes extend.
- {doc}`kyber`: the optimizer a new rule joins, and the invariants it must respect.
- {doc}`execution`: the tiers a new operator has to agree across.
- {doc}`testing-strategy`: the oracles every extension is proven against.
