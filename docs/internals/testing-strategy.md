...
- {doc}`execution`: the sequential, parallel, and JIT paths under test.
- {doc}`kyber`: the passes the differential tests guard.
- {doc}`extending`: where a new operator, rule, or format adds its own coverage.
