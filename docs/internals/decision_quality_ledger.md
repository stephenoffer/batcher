...
## Join strategy — the build side after a swap

| # | Cat | Improvement |
|---|-----|-------------|
| D78 | bug | **After a build-side swap the decision reported the *probe* as the build.** `adaptive_build_side` reassigns `node` to the swapped join, then read `build_side = node.left if swap else node.right` — the pre-swap convention applied to a post-swap node, when the code's own comment four lines above says "after any swap, the right input is the build side". So `_bounded_build_bytes` measured the wrong input and `max(build_bytes, bounded)` inflated the reported figure to it. Measured on a 5,000-row left joined to a 20,000,000-row right — the ordinary small-dimension-on-the-left shape, so the swap fires on essentially every star-schema join written in that order — the build is the 40,000-byte left and the decision reported **320,000,000**. Held the build fixed and grew the probe 200x: the figure tracked the probe exactly. |
| D79 | bug | Two consumers read that figure and both were wrong. `BuildSideDecision.build_bytes` exists so the learned broadcast crossover can be fitted **in bytes** — so the fit was against the wrong side entirely. And the sort-merge memory guard (`build_bytes > smb`) is what decides a hash table would strain memory, so an inflated figure steers a join whose real build is 40 KB toward the bounded-memory merge: the 7x penalty `_SORT_MERGE_MEMORY_SHARE` was written to prevent, arriving through the number it reads. |
| D80 | perf | A/B'd on the queries that exercise it, because a full-suite reading under concurrent load first suggested a regression and turned out to be noise (DuckDB's own times moved with it). Same binary, one line differing: **q9 56.1 -> 53.7 ms, q13 46.0 -> 39.2 ms.** The fix is faster, and the first reading was worth not believing. |
