...
### #40 — The key types grace spilling is verified on, pinned

Not a fix — three properties that held and now cannot quietly stop holding. Each was
*measured* rather than assumed, and each is the kind of thing a throughput optimization
breaks silently.

- **A dictionary key whose dictionary differs per morsel.** This is the normal shape, not a
  corner: every Parquet row group carries its own dictionary, so index 0 means one string in
  the first morsel and another in the second. Spilling routes rows by a hash of the group
  key, and a hash of the *indices* would send the same logical key to two buckets and
  finalize it twice — a wrong answer with no error, on the encoding that exists for exactly
  this kind of column. Arrow's row encoding dereferences the dictionary, so it is correct
  today. The test exists because the partitioner has raw-hash fast paths for `Int64` and for
  mixed string/binary keys, added for throughput, and a dictionary one would break this while
  the result stays correct for as long as every morsel happens to share a dictionary — which
  is what a hand-built fixture usually does.
- **Nested keys.** A struct key and a list key both spill to the same relation the in-memory
  path produces (verified interactively before the tests were written).
- **Decimal and timezone-aware timestamp keys.** They take the general row-encoding path
  rather than either fast path, so they are the types most likely to be missed when a new one
  is added — and a key type that cannot be partitioned is a query that fails only once it
  grows large enough to spill.

---

## A note on this file

It has been silently removed from the working tree twice mid-session, by something outside the
commits that touch it (only they appear in its history). Both times an append recreated it
holding one entry, and the commit that carried that entry also carried the deletion of every
earlier one. If you find it short, restore it from the last commit whose diff for this path is
an addition — do not assume the entries were retired.
