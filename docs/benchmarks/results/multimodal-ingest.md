...
- {doc}`/architecture/deep-dives/memory/tensor-columns`: the `fixed_shape_tensor` representation fix 2 turned on.
- {doc}`/architecture/deep-dives/operators/morsel-parallelism`: the pool sizing that fix 1 corrected.
- {doc}`/user-guide/transform/columns/udfs`: the cost of the Python boundary, and how to avoid paying it.
- {doc}`/benchmarks/methodology`: the machines; the 96-core and 8xT4 tables above are not comparable with each other.
