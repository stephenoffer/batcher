...
:::{note}
The two tables below were measured on a single 96-core CPU node. Everything above them was measured on an 8xT4 cluster. Read them as separate results: an img/s figure from one machine and a clip/s figure from another are not on the same axis, however tempting the arithmetic is. {doc}`/benchmarks/methodology` has the hardware per family.
:::
...
- {doc}`/benchmarks/results/multimodal-ingest`: the image, point-cloud, audio, and video pipelines in full, including the regression that started the work.
- {doc}`/architecture/deep-dives/distribution/gpu-execution`: stage overlap and the warm pool, from the inside.
- {doc}`/architecture/deep-dives/memory/tensor-columns`: the representation the point-cloud result falls out of.
