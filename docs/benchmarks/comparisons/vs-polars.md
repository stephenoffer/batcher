...
- {doc}`/architecture/deep-dives/operators/sort-internals` for the fused top-N heap behind the 50x, and the sample-sort that narrowed the float-sort loss.
- {doc}`/architecture/deep-dives/operators/window-internals` for why `lag()` is ahead and the partitioned `SUM` is behind.
