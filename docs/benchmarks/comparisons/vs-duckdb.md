...
Two honest caveats. Stage-boundary re-optimization is off for queries under 20M input rows, so most small queries never use it. And it's the same mechanism and granularity as Spark AQE, not something finer. See {doc}`/benchmarks/results/scaling`.
...
- {doc}`/architecture/deep-dives/operators/join-algorithms` for the operator the TPC-H gap lives in.
