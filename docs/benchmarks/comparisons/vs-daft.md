...
TPC-H at scale factor 1, 16 cores, re-measured 2026-07-18. The ratio is `batcher / daft`, so **above 1 means Daft is faster**. Batcher is faster on 11 of the 18 queries Daft answers correctly. {doc}`/benchmarks/results/tpch` carries the full per-query table.
...
- {doc}`/benchmarks/comparisons/vs-duckdb` for the same join gap against the other native engine.
