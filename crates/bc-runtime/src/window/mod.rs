...
// The kernels this module dispatches to. Only `frame` is public: `bc_interp::ops` maps the
// IR's frame vocabulary onto its types. The rest are reachable from anywhere inside the
// `window` package (a child module may read its ancestor's private items) and from nowhere
// else, which is what keeps the dispatch in this file the single entry point.
pub mod frame;
...
pub enum WindowFn {
    ...
    /// take no frame. → Float64. See [`crate::window::series`].
    EwmMean,
    EwmVar,
    EwmStd,
    /// Draw a straight line across each interior run of nulls, weighted by ordered
    /// row position (Polars `interpolate`). → Float64.
    Interpolate,
...
impl WindowFn {
    ...
    fn is_aggregate(self) -> bool {
        ...
        ) || crate::window::agg::is_extended_aggregate(self)
    }
    ...
    /// The whole-prefix recurrences in [`crate::window::series`]. They are neither
    /// reducing aggregates nor positional selections: each row's answer is a function of
    /// every earlier row in the order, carried in a running state. That rules out a
    /// frame (there is no subset of rows to aggregate) and rules out peers (an EWM
    /// weight is a function of position, not of ties).
...
pub struct WindowCall {
    ...
    pub frame: Option<crate::window::frame::Frame>,
    /// EWM smoothing factor in `(0, 1]`; required by `ewm_mean`/`ewm_var`/`ewm_std`
    /// and ignored by every other function.
    pub alpha: Option<f64>,
    /// EWM half-life in the ORDER BY key's own units (microseconds for a temporal key).
...
pub(crate) fn window_serial(
            ...
            .all(|c| c.frame.is_none() && c.func.is_aggregate())
    {
        ...
        return funcs
            .iter()
            .map(|call| {
                crate::window::partition_agg::broadcast_partition_aggregate(
                    call.func,
                    &group_ids,
                    num_groups,
                    call.values.as_ref(),
    ...
    // A value-based RANGE frame reads the single ORDER BY key's *values*. SQL allows
    // exactly one order key with such a bound, because the bound is arithmetic on the key
    // and there is no arithmetic on a tuple; a key with no distance (a string) declines the
    // frame rather than silently falling back to the peer frame, which would answer a
    // different question with no error.
    ...
    let range_order = if needs_range_order {
        ...
        match crate::window::frame::RangeOrder::read(key, opts.descending)? {
            Some(o) => Some(o),
            None => {
                return Err(RuntimeError::RangeFrameKeyNotMeasurable {
                    dtype: key.data_type().to_string(),
                })
            }
        }
    } else {
    ...
    for call in funcs {
        out.push(match call.func {
            ...
            f if f.is_value() => crate::window::frame::framed_value(
                f,
                &ordered,
                require(call.values.as_ref(), f)?,
                call.offset,
            ...
            f if call.frame.is_some() => crate::window::frame::framed_aggregate(
                f,
                &ordered,
                require(call.values.as_ref(), f)?,
                call.frame.expect("frame present"),
            ...
            f if order_keys.is_empty() => crate::window::partition_agg::partition_aggregate(
                f,
                &partitions,
                call.values.as_ref(),
                num_rows,
...
/// Dispatch the whole-prefix series recurrences to [`crate::window::series`].
///
/// The EWM family needs a smoothing factor, which the Python builder validates into
/// `(0, 1]` before it reaches the IR. A missing or out-of-range one is therefore a
/// directly-constructed-IR case, and it errors rather than defaulting: silently picking
/// an alpha would return a plausible curve for a query that asked for a different one.
fn series_window(
    func: WindowFn,
    ordered: &[Vec<usize>],
    call: &WindowCall,
    range_order: Option<&crate::window::frame::RangeOrder>,
    num_rows: usize,
) -> Result<ArrayRef, RuntimeError> {
    let values = require(call.values.as_ref(), func)?;
    // A half-life decays by the order key's elapsed value, which the caller has already
    // read (`needs_range_order`); an alpha decays by row position and reads nothing.
    if let Some(hl) = call.half_life {
        if func != WindowFn::EwmMean || !(hl.is_finite() && hl > 0.0) {
            return Err(RuntimeError::UnsupportedWindow {
                func: func.name().to_string(),
                dtype: "a positive half-life, which only ewm_mean takes".to_string(),
            });
        }
        let order =
            range_order.ok_or_else(|| RuntimeError::RangeFrameNeedsOneOrderKey { got: 0 })?;
        return crate::window::series::ewm_by_window(ordered, values, hl, order, num_rows);
    }
    match func {
        WindowFn::Interpolate => {
        ...
        _ => {
            let alpha = call
                ...
                .ok_or_else(|| RuntimeError::UnsupportedWindow {
                    func: func.name().to_string(),
...
fn value_window(
    ...
    if func.is_fill() {
        return crate::window::fill::fill_window(func, ordered, values, num_rows);
    }
