//! Explicit window frames — the sliding aggregates computed over them.
//!
//! [`bounds`] decides *which* rows each output row's frame covers; this module computes the
//! aggregate over them. The default frame (`RANGE UNBOUNDED PRECEDING TO CURRENT ROW`, with
//! peer-tie semantics) is handled by `window::running_aggregate` instead; everything with an
//! explicit `BETWEEN` lands here. Only the aggregate functions and the positional value
//! functions (`first_value`/`last_value`/`nth_value`) take a frame.
//!
//! Both frame edges are non-decreasing in the row position, so the frame only ever slides
//! right — it is a FIFO queue. Every kernel exploits that to run in **one pass**:
//! `count`/`sum` over integers keep a running accumulator (drop the leaving row, add the
//! entering one — O(n)); `sum`/`avg` over floats use a [`FifoSum`] (a two-stack sliding
//! aggregate that never subtracts, because subtracting floats is catastrophically
//! unstable); `min`/`max` keep a monotonic deque (O(n) amortized). No frame is rescanned.
...
pub fn framed_aggregate(
    ...
    match func {
        ...
        // The folds `agg` owns get a frame through the same two-stack slide, so
        // this arm is now only reached by an aggregate with no sliding form at all.
        other if crate::window::agg::is_extended_aggregate(other) => crate::window::agg::framed(
            other,
            ordered,
            values,
            frame,
...
// `nth` is the eighth argument and belongs to exactly one of the three functions this
// serves, so folding it into a struct would put a field nobody reads in front of two
// callers. The same trade the ASOF entry points make.
...
mod tests {
    ...
    fn huge_frame_offsets_saturate_not_overflow() {
        ...
        let s = framed_aggregate(WindowFn::Sum, &ordered, &values, f_end, None, None, len).unwrap();
        assert_eq!(
    ...
    fn rows_frame_sliding_sum_and_avg() {
        ...
        let s = framed_aggregate(WindowFn::Sum, &ordered, &values, frame, None, None, 5).unwrap();
        let s = s.as_primitive::<Int64Type>();
        assert_eq!(s.values(), &[10, 30, 50, 70, 90]);

        // avg over the same trailing pair: 10, 15, 25, 35, 45.
        let a = framed_aggregate(WindowFn::Avg, &ordered, &values, frame, None, None, 5).unwrap();
    ...
    fn rows_frame_centered_min_max_count() {
        ...
        let mn = framed_aggregate(WindowFn::Min, &ordered, &values, frame, None, None, 5).unwrap();
        assert_eq!(mn.as_primitive::<Int64Type>().values(), &[1, 1, 1, 3, 3]);
        let mx = framed_aggregate(WindowFn::Max, &ordered, &values, frame, None, None, 5).unwrap();
        assert_eq!(mx.as_primitive::<Int64Type>().values(), &[5, 9, 9, 9, 7]);
        let c = framed_aggregate(WindowFn::Count, &ordered, &values, frame, None, None, 5).unwrap();
    ...
    fn groups_frame_aggregates_by_peer_group() {
        ...
        let s =
            framed_aggregate(WindowFn::Sum, &ordered, &values, f, Some(&rows), None, 5).unwrap();
        ...
        let s2 =
            framed_aggregate(WindowFn::Sum, &ordered, &values, f2, Some(&rows), None, 5).unwrap();
        ...
        let s3 =
            framed_aggregate(WindowFn::Sum, &ordered, &values, f3, Some(&rows), None, 5).unwrap();
    ...
    fn sliding_float_sum_is_exact_over_large_magnitudes() {
        ...
        let s = framed_aggregate(WindowFn::Sum, &ordered, &values, frame, None, None, 4).unwrap();
        assert_eq!(
            s.as_primitive::<Float64Type>().values(),
            &[1e16, 1e16, 2.0, 2.0]
        );
        // AVG shares the same accumulator, so it must be exact too: (1+1)/2 = 1.0.
        let a = framed_aggregate(WindowFn::Avg, &ordered, &values, frame, None, None, 4).unwrap();
    ...
    fn framed_i64_sum_overflow_errors() {
        ...
        let r = framed_aggregate(WindowFn::Sum, &ordered, &values, frame, None, None, 2);
    ...
    fn framed_i64_minmax_does_not_spuriously_overflow_on_sum() {
        ...
        let mn = framed_aggregate(WindowFn::Min, &ordered, &values, frame, None, None, 3).unwrap();
        assert_eq!(mn.as_primitive::<Int64Type>().values(), &[big, big, big]);
        let mx = framed_aggregate(WindowFn::Max, &ordered, &values, frame, None, None, 3).unwrap();
    ...
    fn framed_i64_sum_does_not_overflow_on_a_frame_that_fits() {
        ...
        let s = framed_aggregate(WindowFn::Sum, &ordered, &values, frame, None, None, 2).unwrap();
        assert_eq!(s.as_primitive::<Int64Type>().values(), &[i64::MAX, 1]);
        let a = framed_aggregate(WindowFn::Avg, &ordered, &values, frame, None, None, 2).unwrap();
        assert_eq!(
        ...
        for &start in &bounds {
            for &end in &bounds {
                ...
                for func in [
                    ...
                    let got =
                        framed_aggregate(func, &ordered, &values, frame, None, None, n).unwrap();
        ...
        for &start in &bounds {
            for &end in &bounds {
                ...
                for func in [
                    ...
                    let got =
                        framed_aggregate(func, &ordered, &values, frame, Some(&rows), None, n)
                            .unwrap();
