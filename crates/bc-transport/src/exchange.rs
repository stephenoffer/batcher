...
pub(crate) async fn credit_exchange_shard(
    ...
    credit_exchange_group_shard(client, &ticket.to_string(), credits, token, shard, nshards).await
}

/// The separator joining several tickets into one stream's name on the wire.
///
/// A [`ShuffleTicket`] renders as digits and `/`, so a comma cannot occur inside one and
/// splitting the name again on the server is unambiguous.
pub(crate) const TICKET_SEP: &str = ",";

/// Render a group of tickets as the one stream name the server splits again.
///
/// A single-ticket group renders as that ticket alone, so an un-grouped fetch is
/// wire-identical to what it always was.
#[must_use]
pub fn group_name(tickets: &[ShuffleTicket]) -> String {
    tickets
        .iter()
        .map(ShuffleTicket::to_string)
        .collect::<Vec<_>>()
        .join(TICKET_SEP)
}

/// As [`credit_exchange_shard`], but `name` may be a [`group_name`] naming several buckets
/// served over the one stream.
///
/// A hash shuffle cuts each mapper's output into one bucket per reducer, so the bucket count
/// grows as `workers^2` while every bucket shrinks. One stream per bucket then pays a stream
/// setup — a round trip before the first byte, and a fresh encode pipeline that never fills —
/// for a transfer too small to amortize it, and that cost grows with the cluster. Grouping
/// decouples the two: the consumer chooses how many streams to run and packs the buckets into
/// them, so the stream count is a constant rather than a function of the cluster's width.
pub(crate) async fn credit_exchange_group_shard(
    client: &mut FlightClient,
    name: &str,
    credits: u32,
    token: Option<&str>,
