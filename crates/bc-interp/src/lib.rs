...
pub(crate) use union_coerce::coerce_union_branches;
...
fn exec_seq(
    ...
    match plan {
        ...
        RelOp::Distinct {
            ...
            // first `k` rows in first-seen order — `assign_groups` orders representatives by
            // first sighting — so this is an independent statement of the same semantics, and
            // an independent statement is what makes it worth testing against.
            let out = vec![match limit {
                Some(k) if deduped.num_rows() > *k => deduped.slice(0, *k),
                _ => deduped,
            }];
            record_breaker(
                m,
