...
//!
//! The four modules here are the two directions of the same contract plus the zone shift
//! between them. `date` pulls fields *out* of a Date/Timestamp (and owns the truncation
//! and offset arithmetic that go with it); `make` builds one *from* integer parts or an
//! epoch count; `text` renders one as text and reads it back; `timezone` moves an instant
//! between zones.
//!
