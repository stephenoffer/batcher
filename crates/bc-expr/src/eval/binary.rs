...
pub(crate) fn eval_binary(op: BinaryOp, l: &ArrayRef, r: &ArrayRef) -> Result<ArrayRef, ExprError> {
    ...
    //
    // `||` is exempt, because it is not a numeric operator: it renders each operand as
    // text on its own terms, which is what DuckDB does (`SELECT 1 || 1.5` is `'11.5'`,
    // not `'1.01.5'`). Coercing first made the two operands agree on a numeric type
    // *before* rendering, so an integer beside a float printed as `'1.0'`, and once the
    // lattice learned that a boolean widens into a number for a UNION, `bool || double`
    // started printing `'1.0'` where DuckDB prints `'true'`. The Concat arm below casts
    // both sides to Utf8 itself, so it never needed this pass.
    let (l, r) = if matches!(op, Concat) {
        (l.clone(), r.clone())
    } else {
        coerce_numeric(l, r)?
    };
