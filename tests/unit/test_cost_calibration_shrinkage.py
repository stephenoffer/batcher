...
def test_shrinkage_is_symmetric_under_inversion():
    ...
    high = shrink(prior * 10.0, prior, n, strength)
    low = shrink(prior / 10.0, prior, n, strength)
...
def test_shrinkage_is_monotone_in_the_evidence():
    ...
    values = [shrink(measured, prior, n, 10.0) for n in (1, 2, 5, 20, 100)]
...
def test_a_non_positive_measurement_still_blends():
    ...
    assert shrink(0.0, 2.0, 10, 10.0) == pytest.approx(1.0)
