...
# The fill returns `(workers, reserved_cpus, compute_cpus)`. The middle figure is what a
# bundle holds and the last is what the worker's engine may open threads over; they differ
# by the one core `_headroom_grant` leaves schedulable on every node.
def test_head_with_cores_is_excluded(monkeypatch):
    ...
    assert ex._cluster_fill_workers() == (3, 15.0, 16.0)  # 3 workers; grant spares a core
...
def test_zero_cpu_head_already_excluded(monkeypatch):
    ...
    assert ex._cluster_fill_workers() == (2, 15.0, 16.0)  # 2 workers; grant spares a core
...
def test_homogeneous_stays_one_worker_per_node(monkeypatch):
    ...
    assert ex._cluster_fill_workers() == (16, 31.0, 32.0)  # still one worker per node
...
def test_heterogeneous_fills_bigger_nodes(monkeypatch):
    ...
    assert ex._cluster_fill_workers() == (24, 31.0, 32.0)  # still 8*1 + 8*2 = 24 workers
...
def test_heterogeneous_head_excluded_from_fill(monkeypatch):
    ...
    assert ex._cluster_fill_workers() == (4, 15.0, 16.0)  # still 1 + 3 = 4 workers
