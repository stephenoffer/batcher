...
from batcher.carbonite.memory import pressure as P
...
def test_the_page_cache_is_subtracted_from_the_cgroup_reading(monkeypatch):
    ...
    monkeypatch.setattr(P, "_cgroup_total_bytes", lambda: int(22.6 * _GIB))
    monkeypatch.setattr(P, "_cgroup_file_cache_bytes", lambda: int(15.3 * _GIB))
    assert P._cgroup_current_bytes() == pytest.approx(7.3 * _GIB, rel=1e-6)
...
def test_anonymous_memory_is_still_counted_in_full(monkeypatch):
    ...
    monkeypatch.setattr(P, "_cgroup_total_bytes", lambda: 8 * _GIB)
    monkeypatch.setattr(P, "_cgroup_file_cache_bytes", lambda: 0)
    assert P._cgroup_current_bytes() == 8 * _GIB
...
def _cache_heavy_host(monkeypatch) -> None:
    ...
    monkeypatch.setattr(P, "_cgroup_total_bytes", lambda: int(24.3 * _GIB))
    monkeypatch.setattr(P, "_cgroup_file_cache_bytes", lambda: int(15.3 * _GIB))
    monkeypatch.setattr(P, "total_memory_bytes", lambda: 30 * _GIB)
...
def test_a_genuinely_full_host_still_reports_pressure(monkeypatch):
    ...
    monkeypatch.setattr(P, "_cgroup_total_bytes", lambda: 29 * _GIB)
    monkeypatch.setattr(P, "_cgroup_file_cache_bytes", lambda: 0)
    monkeypatch.setattr(P, "total_memory_bytes", lambda: 30 * _GIB)
    cfg = Config()
    with config_context(cfg):
...
def test_no_cgroup_leaves_the_reading_absent(monkeypatch):
    ...
    monkeypatch.setattr(P, "_cgroup_total_bytes", lambda: None)
    assert P._cgroup_current_bytes() is None
...
def test_the_engine_envelope_beats_a_quiet_host(monkeypatch):
    ...
    monkeypatch.setattr(P, "_cgroup_total_bytes", lambda: 1 * _GIB)
    monkeypatch.setattr(P, "_cgroup_file_cache_bytes", lambda: 0)
    monkeypatch.setattr(P, "total_memory_bytes", lambda: 30 * _GIB)
