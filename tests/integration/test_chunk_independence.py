"""Chunk-independence: a result must not depend on how the input was batched.

Every operator is checked here against a 1-chunk vs many-chunk baseline, on one node.

**This file used to be called `test_partition_independence.py` and to claim it pinned "the
single most important invariant for distribution".** It does not, and the difference matters.
Re-chunking an in-memory source inside one process shares one hash table, one combine order,
one accumulation topology and one RNG; distributed execution shares none of those. It adds a
network shuffle with its own partitioning function, per-worker partial aggregation, a
hierarchical combine, and recompute on worker loss. A query can be perfectly chunk-independent
and still wrong across a shuffle — which is exactly the failure mode `CLAUDE.md` names, a
distributed `GROUP BY` on a float key splitting one group in two.

So this is a real and necessary property, stated as what it is: the *precondition* for
distribution, not the proof of it. The proof lives in `tests/differential/
test_diff_distributed_operator_matrix.py` and the `test_distributed_*` files, which need a
cluster and therefore skip in CI — `just lint-skips` counts how many.
"""
