...
@pytest.mark.parametrize("path", sorted(PATHS))
@pytest.mark.parametrize("how", sorted(_JOIN_SQL))
def test_update_with_no_shared_column_keeps_the_left_values(duck, how, path):
    """`other` sharing no non-key column updates nothing, and must not raise.

    The verb lowers to a join plus a `with_columns` over the columns the two sides share.
    With no overlap that list is empty, and `with_columns()` rejects an empty projection --
    so the shape Polars answers by returning the left values unchanged raised
    ``with_columns() requires at least one column`` instead. The join still has to run: `how`
    decides which rows survive whether or not a value moves, which is why this is not simply
    a no-op returning `ds`.
    """
    target = pa.table({"id": [1, 2, 3], "x": [10, 20, 30]})
    source = pa.table({"id": [1, 2], "w": [99, 98]})
    duck.register("t", target)
    duck.register("s", source)
    ds = bt.from_arrow(target).update(bt.from_arrow(source), on="id", how=how)
    out = PATHS[path](ds)
    assert out.column_names == ["id", "x"]
    assert_same(out, duck.sql(f"SELECT t.id, t.x FROM t {_JOIN_SQL[how]} s ON t.id = s.id"))
