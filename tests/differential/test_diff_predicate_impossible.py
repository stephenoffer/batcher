...
_IMAGE_REFUTED = [
    ...
    (lambda: col("ts").dt.weekday() == 0, "extract(isodow FROM ts) = 0"),
    ...
    (lambda: col("s").str.upper() == "ab", "upper(s) = 'ab'"),
    (lambda: col("s").str.lower() == "AB", "lower(s) = 'AB'"),
    # The counting functions and the two lower-bounded float ones.
    (lambda: col("s").str.octet_length() < 0, "strlen(s) < 0"),
...
_IMAGE_SATISFIABLE = [
    ...
    (lambda: col("ts").dt.weekday() == 1, "extract(isodow FROM ts) = 1"),
    (lambda: col("ts").dt.weekday() == 7, "extract(isodow FROM ts) = 7"),
