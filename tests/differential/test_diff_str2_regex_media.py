...
def t(duck):
    tbl = pa.table(
        {
            "s": pa.array(
                ["ab", "xaby", "abab", "aXbXc", "no-match", "", "héllo", None]
            ),
        }
    )
    duck.register("t", tbl)
...
def test_levenshtein_is_byte_based(duck):
    ...
    tbl = pa.table(
        {"s": pa.array(["héllo", "café", "日本", "abc", "", "naïve", None])}
    )
    ...
    expected = duck.sql(
        "SELECT levenshtein(s, 'abc') to_abc, levenshtein(s, 'cafe') to_e FROM lev"
    )
    assert_same(out, expected)
...
def test_to_tensor_rejects_oversized_product():
    ...
    with pytest.raises(Exception):  # noqa: B017, PT011 - clean engine error, not a crash
        bt.from_arrow(tbl).select(
            t=col("img").image.to_tensor(40_000, 40_000)
        ).collect()
    # A bounded request still succeeds (undecodable bytes → null row, not an error).
    out = (
        bt.from_arrow(tbl)
        .select(t=col("img").image.to_tensor(8, 8))
        .collect()
    )
