...
checked against DuckDB; `current_timestamp`/`current_date` bind a literal at
plan-build time, so they are unit-checked for shape (not differential — wall-clock
isn't reproducible).
"""
...
def test_current_timestamp_and_date_bind_literals():
    # Bound once at build time → a constant column, equal across all rows.
    ds = bt.from_pydict({"x": [1, 2, 3]})
    n = ds.select(t=current_timestamp()).collect().to_pydict()["t"]
