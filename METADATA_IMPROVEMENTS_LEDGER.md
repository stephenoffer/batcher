...
shipped, because shipping it needs a measurement this session could not take honestly.

**Measured spill throughput.** `kyber.storage_cost.spill_device_factor` prices a spilled byte
from a static table keyed on the device class read out of `/sys` — 10x for network, 30x for
rotational, local flash the omitted baseline "because that is where the spill cost term was
calibrated". Meanwhile Core records `spill_bytes`, `io_write_bytes`, and `t_op_ms` per
operator, from which the *actual* sustained spill throughput of this machine's spill directory
follows directly. That is the same shape as the source read-throughput work in M46, and the
same argument applies: a table of device trivia is a prior, and a measurement of the device
in front of you beats it.

It is not shipped because the honest version needs a spill-heavy benchmark to sit beside it.
The static table is *calibrated against* the cost term it multiplies, so replacing it is a
re-tuning of two coupled numbers, not a drop-in sharpening — exactly the trap
`SourceStatistics.content_byte_size` documents from the width estimator, where a *more
accurate* figure produced *worse* plans because the threshold reading it had been tuned
against the old one. Doing it right means running `bench-ops` and
`tests/integration/test_spilling.py` under real memory pressure and re-tuning both together.

**Measured RSS against the modelled peak.** `OperatorFeedback.peak_rss_bytes` is the
ground-truth memory high-water from `getrusage`, added precisely because `m_peak_bytes` is an
Arrow-size *model* that cannot see transient scratch, allocator fragmentation, or off-pool
buffers. Carbonite's learned memory model — the component whose job is to not OOM — fits on
the model figure and never looks at the measurement. The ratio between them is a
fragmentation factor worth learning per family.

It is not shipped because the field is a *delta* in a process-wide counter: it is non-zero
only for the operator that set a new high-water, and under concurrency it attributes another
operator's growth to whichever one happened to be running. Fitting a safety multiplier on
that risks inflating every reservation on a mis-attributed sample, which costs concurrency on
every query to guard against an OOM that may not exist. It needs the per-operator attribution
tightened first, or a paired A/B under memory pressure.

**Footer-derived per-column widths.** A Parquet footer carries `total_uncompressed_size` and
`num_values` per column chunk, which is an `avg_bytes` for free — and `avg_bytes` today comes
only from executing a query. Not shipped, and this one is worth recording as a *near miss*:
the footer figure is the **encoded** size, so a dictionary-encoded string column measures far
narrower than its materialized Arrow width, and feeding that to the width estimator would
understate exactly the columns it most matters for. `content_byte_size` already documents the
whole-relation version of this trap, measured at q9 going from 0.84x to 1.60x against DuckDB
on a width that was *more* accurate. The right source for a materialized width is the
measurement Core already takes.

## A note on this file

It has already been deleted from the working tree once, out of band, and picked up as a
deletion by an unrelated commit. Two other ledgers in this repo carry the same scar
(`docs(ledger): restore the spill ledger again, and record that it keeps vanishing`). If it
is missing, restore it from the newest commit that still has it rather than starting over.
