...
def autocorrelation(ds: Dataset, column: str, k: int, *, order_by: str) -> float:
    ...
    if ss_total == 0:
        return float("nan")
    lagged = ds.with_columns(__bt_lag=lag(col(column), k).over(order_by=[order_by]))
    overlap = lagged.filter(col("__bt_lag").is_not_null())
    centered = col(column) - lit(mean_value)
    centered_lag = col("__bt_lag") - lit(mean_value)
    numerator = overlap.agg(s=sum_(centered * centered_lag)).collect().column("s")[0].as_py()
    return float(numerator) / ss_total
...
def autocorrelations(ds: Dataset, column: str, lags: int, *, order_by: str) -> dict[int, float]:
    ...
    Examples:
        .. doctest::
            ...
            {1: 0.5, 2: 0.057}
    """
    return {k: autocorrelation(ds, column, k, order_by=order_by) for k in range(1, lags + 1)}
