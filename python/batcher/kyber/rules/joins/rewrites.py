...
def _right_unique_on_keys(join: Join, ctx: OptimizerContext) -> bool:
    ...
    #
    # The gate is `ndv_is_exact` — the ndv's *own* provenance — not the bundle's
    # `provenance`. A Parquet column carries EXACT bounds *and* a measured HyperLogLog
    # ndv, so the bundle reads EXACT while the count itself is a ~1%-error sketch.
    # Reading the bundle tag turned that estimate into a uniqueness *proof*: HLL
    # overestimates above ~50k distinct values, so `ndv (60163.5) >= rows (60000)` on a
    # table that is not unique, and the three rules gated on this then dropped a LEFT
    # JOIN's duplicate matches and pushed an aggregate below a fanning-out join —
    # halving SUM and COUNT. Uniqueness must be proven, never estimated.
    if len(join.right_keys) == 1:
        stats = ctx.estimator.estimate(right)
        col = stats.column(join.right_keys[0])
        if stats.rows_exact and col.ndv_is_exact and col.ndv is not None and col.ndv >= stats.rows:
            return True
    return False
