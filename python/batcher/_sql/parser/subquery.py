...
def _apply_in_subquery(tr, ds: Dataset, node, *, negate: bool) -> Dataset:
    ...
    if where is not None:
        for leaf in _split_and(where.this):
            pair = _correlation_pair(leaf, local)
...
def _correlation_pair(leaf, local: set[str]):
    """If `leaf` is `outer.col = inner.col`, return `(outer_col, inner_col)`.

    Exactly one side must be qualified by a table outside `local` (the outer
    reference); the other is local. Otherwise return None (a local predicate).
    """
    from sqlglot import expressions as exp
    ...
    if not (isinstance(lhs, exp.Column) and isinstance(rhs, exp.Column)):
        return None
    lhs_outer = bool(lhs.table) and lhs.table not in local
    rhs_outer = bool(rhs.table) and rhs.table not in local
