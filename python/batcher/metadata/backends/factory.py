"""Construction of a `MetadataBackend` from its config name.

The name-to-class mapping and the default on-disk location live here rather than in the
package `__init__`, which is a re-export facade. Every caller still imports these from
`batcher.metadata.backends`, which re-exports them.
"""
