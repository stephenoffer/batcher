...
def rank_rows(
    ...
    keys = _order_keys(order_by, api)
    rank = row_number().over(order_by=keys, descending=descending)
    return ds.with_columns(**{name: rank})


def _order_keys(order_by: OrderSpec, api: str) -> list[Any]:
    """The ordering keys `order_by` names, refusing an empty one.

    Both ranking forms below need the same list and owe the same error, so neither
    restates it: a positional verb with no `order_by` is asking a relation for an order
    it does not have, whichever way the position is then computed.

    Args:
        order_by: The ordering keys, one or several.
        api: The public method name, quoted in errors.
...
def position_rows(
    ...
    """`ds` sorted by `order_by` with a 1-based position column `name`.

    The same numbering `rank_rows` produces, reached by sorting and indexing rather than by
    a `row_number` window, and the two are interchangeable only in what they compute. A
    global window (one with no PARTITION BY) that is later *filtered* has no distributed
    form -- each ordered bucket would have to read rows it does not hold -- so the
    distributed executor refuses that plan shape outright. Sorting distributes, and
    `with_row_index` numbers the sorted result, which is why `split` is built this way and
    `split_at_indices` already was.

    Ties in `order_by` are broken arbitrarily here exactly as they are by `row_number`, so
    a caller that needs a stable answer must give `order_by` a key that is unique.

    Args:
        ds: The relation to number.
        order_by: The ordering keys.
        descending: Order every key, or each one, largest first.
        name: The position column to add.
        api: The public method name, quoted in errors.

    Returns:
        `ds` in `order_by` order, with the 1-based position column appended.
    ...
    return ds.sort(*keys, descending=desc).with_row_index(name, offset=1)
...
def build_update(
    ...
    # `other` may share no non-key column with `ds`, and then there is nothing to overwrite:
    # Polars returns the left frame's values unchanged rather than failing. The join still
    # runs, because `how` decides which rows survive whether or not a value moves.
    updated = joined.with_columns(**new_values) if new_values else joined
    return updated.select(*ds.columns)
