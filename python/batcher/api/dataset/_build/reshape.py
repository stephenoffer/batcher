...
from batcher.api.dataset._build.combine import OrderSpec, position_rows, rank_rows, unused_name
...
def build_split(
    ...
    # Positions come from a sort, not a `row_number` window: a global window that is then
    # filtered -- which is exactly the shape below -- has no distributed form, so building
    # it that way made `split` the one W8 verb that raised under `distributed=True`.
    ranked = position_rows(ds, order_by, descending, name=pos, api="split")
    total = ds.count()
    ...
    for size in sizes:
        ...
        # No re-sort after the filter. `position_rows` has already ordered the rows and a
        # filter keeps them in that order, which is the assumption `split_at_indices` has
        # always made; adding a second sort here is what the distributed executor refuses
        # as an unsupported combination, and it bought nothing single-node either.
        parts.append(ranked.filter(keep).drop(pos))
        start += size
    return parts
