...
body = [
    note(490, 52, "A streaming aggregate keeps one row per group. The checkpoint is what lets a restart pick that up rather than re-read the stream.", anchor="middle"),
    ...
    note(465, MEM_Y + MEM_H + 18, "bounded by the group count, not by how many rows produced it", anchor="middle"),
    ...
    card(RS_X, RS_Y, RS_W, RS_H, "batch 9 runs again, with batch 8's state", "a batch in offsets but not in commits is the one to replay"),
    ...
    note(490, 568, "Combining a base snapshot with the deltas recorded after it reaches the same state a full snapshot would, because combine is associative and commutative.", anchor="middle"),
    note(490, 590, "A windowed aggregate evicts too, and its eviction is always a prefix, so one bound in each entry describes it.", anchor="middle"),
]
