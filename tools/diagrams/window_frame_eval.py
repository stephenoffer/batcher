...
from _authoring import AMBER, AMBER_DEEP, BLUE, BLUE_MID, FONT, GREY, band, label, note, svg, write
...
body: list[str] = [
    ...
    note(490, 62, "The rows are sorted once by the PARTITION BY keys and then the ORDER BY keys. "
                  "The partition keys lead, so", anchor="middle"),
    note(490, 80, "every partition comes out contiguous, and a near-unique key does not cost "
                  "a million tiny sorts.", anchor="middle"),
    note(490, 104, "Each function's output column is then scattered back to the row it came from, "
                   "in the original row order.", anchor="middle"),
...
body += [
    ...
    label(337, CY - 20, "peer group: three rows tied on the ORDER BY key", anchor="middle", size=11.5),
...
body += [
    ...
    note(X0, 300, "stops at this row: pure position arithmetic, so each tied row gets a different answer"),
...
body += [
    ...
    note(490, 438, "GROUPS counts peer groups the way ROWS counts rows. A numeric RANGE offset is a "
                   "binary search over the", anchor="middle"),
    note(490, 456, "key's values rather than a walk over peers, and a null order key frames only its "
                   "own null peer group.", anchor="middle"),
    note(490, 480, "Both frame edges only ever slide right, so a frame is a FIFO queue and no frame is "
                   "ever rescanned.", anchor="middle"),
]
